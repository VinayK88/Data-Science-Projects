...
class RiskOSScienceTest(unittest.TestCase):
    def test_simulator_is_deterministic(self):
        a = generate_cases(n=25, seed=9)
        b = generate_cases(n=25, seed=9)
        self.assertEqual(a, b)

    def test_risky_population_scores_higher_on_average(self):
        cases = generate_cases(n=500, seed=17)
    ...
    def test_threshold_metrics_are_bounded(self):
        result = evaluate_threshold(generate_cases(n=300), 0.70, review_capacity=60)
