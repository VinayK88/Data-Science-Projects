...
Run from the riskos project directory:
...
with tab1:
    ...
    st.write(
        "Cases are ranked by expected loss so a high-exposure 72% risk case can outrank "
        "a 99% risk case with little financial exposure."
    )
    st.dataframe(
        score_df.head(50),
...
with tab3:
    ...
    reference_cases = generate_cases(n=sample_size, fraud_rate=0.14, seed=17)
    ...
    dist_df = pd.DataFrame({"reference": reference_scores, "shifted": shifted_scores})
    st.bar_chart(
        pd.DataFrame(
            {
                "reference": pd.cut(dist_df["reference"], bins=[0, .2, .4, .6, .8, 1], include_lowest=True).value_counts(sort=False),
                "shifted": pd.cut(dist_df["shifted"], bins=[0, .2, .4, .6, .8, 1], include_lowest=True).value_counts(sort=False),
            }
        )
    )
